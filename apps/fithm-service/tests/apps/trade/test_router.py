...
def test_trade_put_add_portfolio(app, business, trade, portfolio):
    with app.test_client() as test_client:
        response = test_client.post(
            f"/api/v1/trades/{trade.id}/portfolios", json={'business_id': business.id,
...
def test_trade_get_with_portfolio_account_account_position(app, business, trade, portfolio_account_account_position):
    with app.test_client() as test_client:
        response = test_client.post(
            f"/api/v1/trades/{trade.id}/portfolios", json={'business_id': business.id,
...
def test_trade_get_instructions(app, business, trade_with_portfolios_accounts):
    with app.test_client() as test_client:
        response = test_client.get(
            f"/api/v1/trades/{trade_with_portfolios_accounts.id}/instructions",
            query_string={"business_id": business.id},
        )
    assert response.status_code == 200
