...
class TradeParser:
    ...
    def parse_update(self, req: Request) -> dict:
        if not self.update:
            ...
            self.update.add_argument(
                "status", type=bool, required=True, location="json"
            )
    ...
    def parse_instructions(self, req: Request) -> dict:
        if not self.instructions:
            ...
            self.instructions.add_argument(
                "portfolio", type=dict, required=True, location="json"
            )
    ...
    def parse_update_portfolios(self, req: Request) -> dict:
        if not self.update_portfolios:
            ...
            self.update_portfolios.add_argument(
                "portfolios", type=list, required=True, location="json"
            )
    ...
    def parse_update_positions(self, req: Request) -> dict:
        if not self.update_positions:
            ...
            self.update_positions.add_argument(
                "positions", type=list, required=True, location="json"
            )
