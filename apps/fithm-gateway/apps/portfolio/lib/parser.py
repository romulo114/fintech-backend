...
class PortfolioParser:
    ...
    def parse_update_account(self, req: Request) -> dict:
        if not self.update_account:
            ...
            self.update_account.add_argument(
                "accounts", type=list, required=True, location="json"
            )
    ...
    def parse_update_model(self, req: Request) -> dict:
        if not self.update_model:
            ...
            self.update_model.add_argument(
                "model_id", type=int, required=True, location="json"
            )
